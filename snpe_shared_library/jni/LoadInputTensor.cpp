...
//Fun def used RB3 app
bool loadInputUserBufferFloatFromBufer(std::unordered_map<std::string, std::vector<uint8_t>>& applicationBuffers,
                         std::unique_ptr<zdl::SNPE::SNPE>& snpe,
                         std::vector<uint8_t>& buffer)
{
    ...
    for (size_t j = 0; j < inputNames.size(); j++) {
        const char *name = inputNames.at(j);
        applicationBuffers.at(name) = buffer;
    }
    return true;
}
